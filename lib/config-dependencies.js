...
module.exports = async function main() {
  try {
    ...
    if (isDevDepsCached) {
      ...
      if (devDepsKit.length) {
        await npmInstall(false, devDepsKit);
      } else {
        console.log(`No dev dependencies installed. You may install it manually.`);
      }
    }
    ...
    let depsKit = _depsKit.split(' ');

    if (depsKit.length) {
      await npmInstall(false, depsKit);
    } else {
      console.log(`No prod dependencies installed. You may install it manually.`);
    }
